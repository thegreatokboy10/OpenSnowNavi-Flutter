...
class TeamStorageService {
  ...
  Future<void> saveCurrentTeamId(String? teamId) async {
    ...
    if (teamId == null) {
      await _prefs!.remove(_currentTeamKey);
    } else {
      await _prefs!.setString(_currentTeamKey, teamId);
    }
  }

  /// 获取本地保存的当前团队ID
  Future<String?> getSavedTeamId() async {
    await _ensureInitialized();
    return _prefs!.getString(_currentTeamKey);
  }

  /// 同步获取本地保存的当前团队ID
  String? getSavedTeamIdSync() {
    return _prefs?.getString(_currentTeamKey);
  }
  ...
  Future<TeamResult> createTeam({
    ...
    await saveCurrentTeamId(team.id);
  ...
  /// 加入团队
  Future<StorageJoinResult> joinTeam({
    ...
    if (result.success && result.data != null) {
      await saveCurrentTeamId(teamId);
      // 检查是否是重新加入（成员已存在）
      ...
      return StorageJoinResult(
          success: true, team: result.data, isRejoin: isRejoin);
    }

    return StorageJoinResult(success: false, error: result.error ?? '加入团队失败');
  }
